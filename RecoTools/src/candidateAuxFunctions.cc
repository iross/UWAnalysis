		...
		const std::vector<int>* pdgIds, bool pdgIdStrict)
{
	...
	for ( reco::GenParticleCollection::const_iterator genParticle = genParticles.begin();
		...
		// require status 0 if lepton
		if (abs(genParticle->pdgId())==11||abs(genParticle->pdgId())==13||abs(genParticle->pdgId())==15) status=0;
		...
		bool closerThanBestMatch = (bestMatch) ? (reco::deltaR(direction, genParticle->p4()) < reco::deltaR(direction, bestMatch->p4())) : true;
		...
		if ( bestMatchMatchesPdgId ) {
			...
			if (closerThanBestMatch || matchesPdgId ) {
