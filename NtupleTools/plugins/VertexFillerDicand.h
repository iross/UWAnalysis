...
template<class T>
reco::TransientTrack getTrackDicand(const T& cand, const TransientTrackBuilder& builder);
template<> 
reco::TransientTrack getTrackDicand<pat::Muon>(const pat::Muon& muon, const TransientTrackBuilder& builder) { return builder.build(muon.innerTrack()); }
template<> 
reco::TransientTrack getTrackDicand<pat::Electron>(const pat::Electron& electron, const TransientTrackBuilder& builder) { return builder.build(electron.gsfTrack()); }
template<>
reco::TransientTrack getTrackDicand<pat::Tau>(const pat::Tau& tau, const TransientTrackBuilder& builder) { 
...
//
template<typename T>
class VertexFillerDicand : public NtupleFillerBase {
	public:
		VertexFillerDicand(){
		}


		VertexFillerDicand(const edm::ParameterSet& iConfig, TTree* t):
			...
			vertexTag_(iConfig.getParameter<edm::InputTag>("vertexTag"))
	{
		...
		void fill(const edm::Event& iEvent, const edm::EventSetup& iSetup)
		{
			...
			if(iEvent.getByLabel(src_,handleT)){
				if (handleT->size()>0){
					transientTracks.push_back(getTrackDicand(*(handleT->at(0).leg1()), *TransientTrackBuilder_));
					transientTracks.push_back(getTrackDicand(*(handleT->at(0).leg2()), *TransientTrackBuilder_));
					z1l1IPpair = IPTools::absoluteImpactParameter3D( getTrackDicand(*(handleT->at(0).leg1()), *TransientTrackBuilder_), primVertex);
					z1l2IPpair = IPTools::absoluteImpactParameter3D( getTrackDicand(*(handleT->at(0).leg2()), *TransientTrackBuilder_), primVertex);
...
typedef VertexFillerDicand<PATMuPair> MuMuVertexFiller;
typedef VertexFillerDicand<PATElecPair> EleEleVertexFiller;
