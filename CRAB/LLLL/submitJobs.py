...
for dataset in datasets:
    ...
    if passes:
        ...
        if datasets[dataset]['url'] == '':
            out.write('farmoutAnalysisJobs  --shared-fs --output-dag-file=/scratch/$USER/DAGs/{tag}/{dataset} --input-dir=root://cmsxrootd.hep.wisc.edu/{path} {dataset}_{tag} $CMSSW_BASE $CMSSW_BASE/src/UWAnalysis/CRAB/LLLL/'.format(tag=tag,dataset=dataset,path=datasets[dataset]['path'])+runFile+'\n')
        else:
            out.write('farmoutAnalysisJobs  --shared-fs --output-dag-file=/scratch/$USER/DAGs/{tag}/{dataset} --input-dbs-path={path} --dbs-service-url={url} {dataset} $CMSSW_BASE $CMSSW_BASE/src/UWAnalysis/CRAB/LLLL/'.format(tag=tag,dataset=dataset,path=datasets[dataset]['path'],url=datasets[dataset]['url'])+runFile+'.py\n')
