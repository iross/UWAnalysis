...
#		massMap[mass]=yieldz[0]-yieldz[1] #get yields - overlap
	...
	#samples=["ZZ4L_pythia"]
	samples=["ZZ4L_pythia_dR04"]
	lines=[]
	for chan in ("muMuTauTau","eleEleTauTau","eleEleEleTau","muMuEleTau","muMuMuTau","eleEleMuTau","eleEleEleMu","muMuEleMu"):
...
def overlap(channel1,channel2, sample, lumi):
	file1 = open(""+sample+"/"+channel1+sample)
	file2 = open(""+sample+"/"+channel2+sample)
...
#			corr = float(fields[5])
			...
			excepts+=1
		#	if (excepts==1):
		#		print "No weight or corr found! Defaulting to 1 * 1!"
			weight=1
			corr=1
