...
for dataset in datasets:
    ...
    if passes:
        if datasets[dataset]['type']=="DATA":
            print dataset
            subprocess.call("cat ZH.py > DATA_{dataset}.py".format(dataset=dataset),shell=True)
        ...
        if datasets[dataset]['url'] == '':
            out.write('farmoutAnalysisJobs --skip-existing-output --output-dag-file=/scratch/iross/DAGs/{tag}/{dataset} --input-dir=root://cmsxrootd.hep.wisc.edu/{path} {dataset}_{tag} $CMSSW_BASE $CMSSW_BASE/src/UWAnalysis/CRAB/ZH/'.format(tag=tag,dataset=dataset,path=datasets[dataset]['path'])+runFile+'\n')
        else:
            out.write('farmoutAnalysisJobs --skip-existing-output --output-dag-file=/scratch/iross/DAGs/{tag}/{dataset} --input-dbs-path={path} --dbs-service-url={url} {dataset} $CMSSW_BASE $CMSSW_BASE/src/UWAnalysis/CRAB/ZH/'.format(tag=tag,dataset=dataset,path=datasets[dataset]['path'],url=datasets[dataset]['url'])+runFile+'.py\n')
