...
void makeSetOfPlots(TFile *f, TString postfix,TString var, float min,float max,bool log,TString canvasName="",TString xtitle = " ref object p_{T}" , TString ytitle = "Efficiency")
{
  ...
  TH1F *dm = f->Get("tauFakeRateMC"+postfix+"/leadingTrackFinding"+var); 
  TH1F *vl = f->Get("tauFakeRateMC"+postfix+"/byVLooseIsolationBeta"+var); 
  TH1F *l = f->Get("tauFakeRateMC"+postfix+"/byLooseIsolationBeta"+var); 
  TH1F *m = f->Get("tauFakeRateMC"+postfix+"/byMediumIsolationBeta"+var); 
  TH1F *t = f->Get("tauFakeRateMC"+postfix+"/byTightIsolationBeta"+var); 

   // TGraphAsymmErrors * eff_dm = new TGraphAsymmErrors;
  
   // eff_dm->Divide(dm,match);
