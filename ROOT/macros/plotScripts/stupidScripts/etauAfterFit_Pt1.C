...
// examples macro
void 
etauAfterFit_Pt1(bool scaled = true, bool log = true)
{
  // defining the common canvas, axes pad styles
  SetStyle();

  // open example histogram file
  TFile* exampleFile = new TFile("more/eleTauPT1.root");

  //load example histograms
  TH1F* data = (TH1F*)exampleFile->Get("eleTau_X/data_obs");
  if(data) {InitHist(data, "#bf{e p_{T} [GeV]}", "#bf{Events}"); InitData(data);} else{std::cout << "can't find hitogram " << "eleTau_SM0/data_obs" << std::endl;}
  ...
  if(log){
    if(scaled) canv->Print("etau_Pt1_LOG.pdf"); else canv->Print("etau_Pt1_LOG.pdf");
    if(scaled) canv->Print("etau_Pt1_LOG.png"); else canv->Print("etau_Pt1_LOG.png");
