...
// examples macro
void 
muTauAfterFit_Pt1(bool scaled = true, bool log = true)
{
  // defining the common canvas, axes pad styles
  SetStyle();

  // open example histogram file
  TFile* exampleFile = new TFile("more/muTauPT1.root");

  //load example histograms
  TH1F* data = (TH1F*)exampleFile->Get("muTau_X/data_obs");
  if(data) {InitHist(data, "#bf{#mu p_{T} [GeV]}", "#bf{Events}"); InitData(data);} else{std::cout << "can't find hitogram " << "muTau_X/data_obs" << std::endl;}
  ...
  if(log){
     canv->Print("mutau_Pt1_LOG.pdf"); 
     canv->Print("mutau_Pt1_LOG.png"); 
