...
int main (int argc, char* argv[]) 
{
  ...
  gROOT->SetStyle("Plain");
   ...
   parser.addOption("qcdFactor",optutl::CommandLineParser::kDouble,"qcd OSLS Ratio",1.15);
   parser.addOption("qcdFactorErr",optutl::CommandLineParser::kDouble,"qcd OSLS Ratio Error",0.08);
   parser.addOption("wenFactor",optutl::CommandLineParser::kDouble,"WEN factor",0.28);
   parser.addOption("wenFactorErr",optutl::CommandLineParser::kDouble,"WEN factor error ",0.017);
   parser.addOption("wtnFactor",optutl::CommandLineParser::kDouble,"WTN Factor",0.23);
   parser.addOption("wtnFactorErr",optutl::CommandLineParser::kDouble,"WTN FactorErr",0.012);
   ...
   parser.addOption("zjftFactor",optutl::CommandLineParser::kDouble,"ZEEFactor2",2.50);
...
//    printf("control region Mt < =%f\n",ttbarLow);
   ...
   printf("OS W mu nu in core  =%f *%f  = %f +- %f \n",osWHigh,parser.doubleValue("wenFactor"),osWLow,osWLowErr);
   
   printf("3. Extrapolate W ->tau nu from W -> munu using MC \n");
   float osWTLow = osWLow*parser.doubleValue("wtnFactor");
   float osWTLowErr = sqrt(osWLowErr*parser.doubleValue("wtnFactor")*osWLowErr*parser.doubleValue("wtnFactor") +osWLow*osWLow*parser.doubleValue("wtnFactorErr")*parser.doubleValue("wtnFactorErr"));
   ...
   printf("SS W mu nu in core  =%f *%f  = %f +- %f \n",ssHigh,parser.doubleValue("wenFactor"),ssWLow,ssWLowErr);

   printf("5. Extrapolate W ->tau nu from W -> munu using MC \n");
   float ssWTLow = ssWLow*parser.doubleValue("wtnFactor");
   float ssWTLowErr = sqrt(ssWLowErr*parser.doubleValue("wtnFactor")*ssWLowErr*parser.doubleValue("wtnFactor") +ssWLow*ssWLow*parser.doubleValue("wtnFactorErr")*parser.doubleValue("wtnFactorErr"));
